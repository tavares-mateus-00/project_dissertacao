...
fundo_preto = np.zeros(img.shape[:2],dtype=np.uint8)
...
def clear_fundo_preto(x,y,jx,jy):
    for coluna in range(x,jx+1,1):
        for linha in range(y,jy+1,1):
            if fundo_preto[linha,coluna] != 0:
                fundo_preto[linha,coluna] = 0
...
def area(x,w,y,h):
    i = 0
    for linha in range(y,y+h,1):
        for coluna in range(x,x+w,1):
            if img[linha,coluna,1] == 255:
                i = i + 1
    print('áREA:', i)
...
def desenho_livre(evento,x,y,flags,param):
    ...
    if evento == cv.EVENT_LBUTTONDOWN and livre == True:
        ...
        if desenho == True and livre == True:
            ...
            cv.line(fundo_preto, (ix,iy), (x,y), linha_cinza[troca_cor], 2)
            ix, iy = x, y
    ...
    if evento == cv.EVENT_LBUTTONDOWN and borracha == True:
        ...
        clear(jx,jy,x,y)
        clear(x,y,jx,jy)
        clear_fundo_preto(x,y,jx,jy)
        clear_fundo_preto(jx,jy,x,y)
...
while True:
    ...
    if k == ord('x') or k == ord('X'):
        ...
        print('REALIZAR CONTORNO')
        ctn,cont = cv.findContours(fundo_preto,cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)
        for (i,c) in enumerate(ctn):
            if cont[0][i][3] == -1:
                cv.drawContours(img,ctn,i,(0,255,0),-1)
                x,y,w,h = cv.boundingRect(ctn[i])
                #cv.rectangle(img,(x-1,y-1),(x+w+1,y+h+1),(255,0,0),1)
                area(x,w,y,h)
