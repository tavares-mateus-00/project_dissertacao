...
def abertura_residuo(a,B,Bc,tipo):
    if tipo == 'reconstrucao':
        ero = mm.erosion(a,selem=B)
        abe = np.int32(mm.reconstruction(ero,a, method='dilation', selem=Bc))
    elif tipo == 'area':
        if Bc.sum()==5:
            conec = 1
        else:
            conec = 2
        abe = mm.area_opening(a,area_threshold=B.sum(),connectivity=conec)
    else: # caso contrario, use a abertura estrutural
        abe = mm.opening(a,selem=B)
    th = a - abe
    th_bin = 1*(th > 0)
...
for imagem in lista:
    ...
    lista_estrutura.append(control_abe_strut)           #1
    lista_estrutura_bin.append(control_abe_strut_bin)   #2
    lista_reconstrucao.append(control_abe_rec)          #3
    lista_reconstrucao_bin.append(control_abe_rec_bin)  #4
    lista_area.append(control_abe_area)                 #5
    lista_area_bin.append(control_abe_area_bin)         #6
    

np.save('abertura_estrutura_s1.npy',lista_estrutura)           #1
np.save('abertura_estrutura_bin_s1.npy',lista_estrutura_bin)   #2 
np.save('abertura_rec_s1.npy',lista_reconstrucao)              #3
np.save('abertura_rec_bin_s1.npy',lista_reconstrucao_bin)      #4
np.save('abertura_area_s1.npy',lista_area)                     #5
np.save('abertura_area_bin_s1.npy',lista_area_bin)             #6
